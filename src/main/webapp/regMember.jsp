...
<%
  ...
  String phone = makeString(u_phone, "-"); //하나의 문자열로 저장 된다 예) 010-1234-5678

  //Mapper를 호출하기전에 Mapper로 전달할 파라미터 객체를 준비
  MemVO mvo = new MemVO();
  mvo.setM_id(u_id);
  mvo.setM_pw(u_pw);
  ...
  //DAO의 함수를 호출하여 DB에 저장 되도록 한다.
  int cnt = MemberDAO.registry(mvo);
  if (cnt > 0){
    //회원가입의 성공한 경우
    //현재 프로젝트(어플리케이션)의 members라는 폴더의 정확한 절대경로를 확인 하자
    String path = application.getRealPath("/members/"+mvo.getM_id());
//    out.println(path);

    //위 절대경로를 가지고 File객체를 생성한 후 디렉토리(폴더)를 생성 하자
